...
import type { PeriodSummary } from '@/types/ai';
import type { EmotionKey, MoodRecord } from '@/types/mood';
import { getEmotionConfig } from '@/config/emotions';

interface Props {
  label: string;
  periodSummary: PeriodSummary | null | undefined;
  records: MoodRecord[];
}

export function InsightPeriodCard({ label, periodSummary, records }: Props) {
  const topEmotions = useMemo(() => {
    const freq = new Map<EmotionKey, number>();
    for (const r of records) {
      for (const e of r.emotions) freq.set(e, (freq.get(e) ?? 0) + 1);
    }
  ...
  if (records.length === 0) {
    return (
      <div className="paper-card rounded-2xl p-6 text-center">
        <p className="text-micro text-ink-400 mb-4 tracking-widest">{label}</p>
        <span className="text-3xl">🌱</span>
        <p className="mt-3 text-caption text-ink-400">这段时间暂无情绪记录</p>
      </div>
    );
  }
  ...
  return (
    <div className="paper-card rounded-2xl p-6">
      <div className="flex items-center justify-between mb-4">
        <p className="text-micro text-ink-400 tracking-widest">{label}</p>
        <span className="text-micro text-ink-400">{records.length} 条记录</span>
      </div>

      {periodSummary?.summary && (
        <p className="text-body text-ink-900 leading-relaxed mb-5">{periodSummary.summary}</p>
      )}
